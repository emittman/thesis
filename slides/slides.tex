...
\begin{table}
\begin{tabular}{ll}
\noindent\parbox[c]{.8\textwidth}{\includegraphics[width=.8\textwidth]{pairs1}} & \pause $\beta_g \sim ?$
\end{tabular}
\end{table}
\onslide<1->
\end{frame}
...
%%%%%%%%%%%%%%
\section[DP intro]{Intro: Dirichlet processes}

\begin{frame}[label=current]
\scalebox{.8}{
  \begin{beamerboxesrounded}[upper=upcol,lower=lowcol,shadow=true]{Stick-breaking representation:}
    $\mathcal{P}$ = a random probability distribution on $\mathbb{R}^p;p\in \mathbb{N}$\\
    ...
    \begin{itemize}
      ...
      \item $Q$ is a probability distribution on $\mathbb{R}^p$
...
{\small
 ...
 \begin{itemize}
  \pause \item $\mu_g \ind N(\eta, \tau^2)$\\
     with prior distributions $\eta \sim N(0, 5^2)$, $\tau^2 \sim \op{IG}(0.5,0.5)$
  \pause \item $\mu_g \ind \pi_1 \op{N}(\eta_1, \tau^2)+\pi_2 \op{N}(\eta_2, \tau_2) + \pi_3 \op{N}(\eta_3,\tau^2)$ with prior distributions \\ $\eta_j \ind N(0,5^2)$, $\tau^2 \sim $\op{IG}(0.5,0.5)$ and $\pi \sim \op{Dir}(1,1,1)$, subject to $\eta_1<\eta_2<\eta_3$.
   \pause \item $\mu_g \ind DP(\alpha Q)$ with $Q \stackrel{d}{=}\op{N}(0,5^2)$, and prior $\alpha \sim \op{G}(3,3/14.2)$
     \end{itemize}
}
\end{frame}
...
\scalebox{.8}{
  ...
  \pause \item $X$ is the model matrix
  \pause \item $W_g^{-1}$ is a known diagonal matrix of precision weights
  \pause \item $\op{DP}(\alpha Q)$ is a Dirichlet process with concentration parameter $\alpha$ and base measure $Q$
  \end{itemize}
  \end{beamerboxesrounded}
  \onslide<1->
}
...
\scalebox{.8}{
  \begin{beamerboxesrounded}[upper=upcol,lower=lowcol,shadow=true]{Stick-breaking representation \citep{sethuraman}:}
    ...
    \begin{itemize}
      ...
      \item $\nu_k = \frac{\pi_k}{1-\sum_{\ell<k} \pi_\ell} \ind \op{Be}(1,\alpha)$
    \end{itemize}
  \end{beamerboxesrounded}
  \onslide<1->
}
...
\begin{frame}%[label=current]
\frametitle{Normalization and mean-variance trend adjustment}
\citep{voom}
\scalebox{.6}{
  \begin{beamerboxesrounded}[upper=upcol,lower=lowcol,shadow=true]{Definitions}
